...
(defn azure-err-ctx [done-fn]
  "Generate monkeypatched Azure context.
  
   Automatically converts between JS and CLJS
   where applicable to allow writing assertions in more idiomatic manner"
  #js {:done (fn [err result] (done-fn err))
       :log  #(println (str "LOG :: " %))})
...
(defn testing-azure-err-wrapper
  [m handler assertions]
  (testing m
    (async done
      ((azure-function-wrapper handler)
         (azure-err-ctx #(do (assertions %) (done)))
         azure-req))))

; both tests works alone but testing boths breaks things
